...
export default function AccessibilityView() {
  const { currentRouteResult, routeServices, t } = useApp();
  const [selectedCategory, setSelectedCategory] = useState<string>('All Services');
  const [selectedFacility, setSelectedFacility] = useState<AccessibilityFacility | null>(null);

  const facilities = routeServices && routeServices.length > 0 ? routeServices : [];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 p-6 rounded-3xl text-white shadow-md">
        ...
        <p className="text-xs sm:text-sm text-purple-100 max-w-2xl mt-1">
          Emergency trauma hospitals, fuel depots, heavy vehicle workshops, and rest zones along {currentRouteResult.sourceCity} ➔ {currentRouteResult.destCity}.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 space-y-3">
          {filteredFacilities.length > 0 ? (
            ...
            <div className="bg-white p-8 rounded-3xl border border-slate-200 text-center text-slate-500">
              No facilities found under this category along the active corridor.
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-6 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm space-y-3">
          <div className="flex items-center justify-between pb-2 border-b border-slate-100 text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-slate-800">
                Corridor Facilities: {currentRouteResult.sourceCity} ➔ {currentRouteResult.destCity}
              </span>
            </div>
          ...
          <div className="p-3 bg-purple-50 rounded-xl text-xs text-purple-900 flex items-center justify-between">
            <span>Tap any service card to inspect location along {currentRouteResult.sourceCity} ➔ {currentRouteResult.destCity}.</span>
          </div>
        </div>
      </div>
    </div>
